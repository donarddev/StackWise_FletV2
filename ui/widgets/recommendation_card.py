...
from ui.theme import caption_style, subheading_style
    ...
    *,
    ...
    if on_view is not None:
        actions.append(
            ft.TextButton(
                ...
                style=ft.ButtonStyle(color=theme["accent_2"]),
            )
        )
    if on_regenerate is not None:
        actions.append(
            ft.TextButton(
                ...
                style=ft.ButtonStyle(color=theme["accent_soft"]),
            )
        )
    ...
    header = ft.Row(
        controls=[
            ft.Column(
                controls=[
                    ft.Text(rec.project_name, style=subheading_style(theme, size=18)),
                    ft.Text(
    ...
    body = ft.Column(
        ...
        controls=[
            header,
            pillars,
            confidence_bar(rec.confidence_score, theme=theme),
        ],
    )
    return glass_card(body, theme=theme)
            ...
            controls=[
                ft.Row(
                    ...
                    controls=[
                        ...
                        ft.Text(title.upper(), style=caption_style(theme, size=11)),
                    ],
                ),
                ft.Text(value, size=18, weight=ft.FontWeight.W_700, color=theme["text"]),
            ],
        ),
    )
