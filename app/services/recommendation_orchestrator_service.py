"""Legacy recommendation orchestrator (UI + persistence path).

Wraps the original knowledge-base scoring pipeline with repository persistence,
confidence, alternatives, and explanation services. The Phase 1 weighted engine
lives in ``recommendation_service.py`` and is invoked via
``generate_recommendation_from_request`` in the controller.
"""
...
class RecommendationOrchestratorService:
    def __init__(
        self,
    ...
    def _build_risk_analysis(self, request: RecommendationRequest) -> list[str]:
        risks: list[str] = []
        if request.timeline in {"Less than 1 month", "1ΓÇô2 months"}:
